...
def formatar_data(data_str):
    """Formata a data para o formato yyyy-mm-dd, removendo horas se existirem."""
    if data_str and isinstance(data_str, str):  # Verifica se é uma string válida
        try:
            return datetime.strptime(data_str[:10], "%Y-%m-%d").date().isoformat()
        except ValueError:
            return "N/A"  # Se não for uma data válida, retorna "N/A"
    return "N/A"
        ...
        while True:
            ...
            if resposta.status_code == 200:
                ...
                for edital in dados:
                    ...
                    # Formatar as datas do edital
                    edital_info["data_publicacao_pncp"] = formatar_data(edital_info.get("data_publicacao_pncp"))
                    edital_info["data_atualizacao_pncp"] = formatar_data(edital_info.get("data_atualizacao_pncp"))
                    edital_info["data_inicio_vigencia"] = formatar_data(edital_info.get("data_inicio_vigencia"))
                    edital_info["data_fim_vigencia"] = formatar_data(edital_info.get("data_fim_vigencia"))
                    edital_info["data_assinatura"] = formatar_data(edital_info.get("data_assinatura"))
                    edital_info["createdAt"] = formatar_data(edital_info.get("createdAt"))  # 🔹 Correção adicionada aqui!
